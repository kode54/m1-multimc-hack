...
The contents of the `lwjgl` directory were automatically generated by the [LWJGL Customize page](https://www.lwjgl.org/customize), picking both `macOS x64` and `macOS arm64` from the `Early Access` tab. The included `glfw` natives were later built by me from the upstream repository, using Xcode 13, with the only change being the removal of the UNSUPPORTED error notice from the SetWindowIcon native implementation for Cocoa, as Minecraft apparently calls that on all platforms and doesn't know what to do with UNSUPPORTED errors.

The `mcwrap.py` script was written by Yusef Napora, updated by me (Christopher Snowhill), and is public domain. Please feel free to fork and improve. [Share and Enjoy!](https://hitchhikers.fandom.com/wiki/Share_and_Enjoy)
